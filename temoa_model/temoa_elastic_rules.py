...
"""
	# consumer_costs = sum(
	    # value(M.Price[p, s, d, dem])
	    # * (value(M.Demand[p, dem]) * value(M.DemandSpecificDistribution[s, d, dem]))**(1.0 / value(M.Elast[p, s, d, dem]))
	    # * ((value(M.MinDemand[p, dem]) * value(M.DemandSpecificDistribution[s, d, dem]))**(1+(-1.0 / value(M.Elast[p, s, d, dem])))
	    # + ((sum(M.V_DemandSegment[p, s, d, dem, z] * value(M.Demand_curve[p,s,d,dem,z])
	    
	    # for z in M.demand_segment))
	    # / (value(M.DemandSegment_bound_value[p, s, d, dem])))) * (1.0/(1+(-1.0 / value(M.Elast[p, s, d, dem]))))
	    
	    # for (p, s, d, dem) in M.DemandConstraint_psdc)
	
	# #Shoib alc's objective function
	# consumer_costs = sum(
	   # M.V_DemandSegment[p, s, d, dem, z]
	   # * value(M.PriceSegment[p, s, d, dem, z]) #* (1.0/(1+(-1.0 / value(M.Elast[p, s, d, dem]))))
	   # for (p, s, d, dem, z) in M.DemandConstraint_psdcz)
	   
	#Shoib alc's objective function
	#consumer_costs = #sum(
	   #value(M.DemandSegment_bound_value[p, s, d, dem])
...
def PeriodWelfare_rule (M, p):
	...
	consumer_costs = sum(
	   value(M.DemandSegment_bound_value[S_p, s, d, dem])
	   * value(M.PriceSegment[S_p, s, d, dem, z])
	   for (S_p, s, d, dem, z) in M.DemandConstraint_psdcz
	   if S_p == p) - sum(
	   M.V_DemandSegment[S_p, s, d, dem, z]
	   * value(M.PriceSegment[S_p, s, d, dem, z])
	   for (S_p, s, d, dem, z) in M.DemandConstraint_psdcz
	   if S_p == p)
	
	return (period_costs - consumer_costs)
