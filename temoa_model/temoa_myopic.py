...
def myopic_db_generator_solver ( self ):
	...
	for i in range(1,len(time_periods)-1):
		print ('Preparing the database for the periods '+str(time_periods[i-1][0])+" and "+str(time_periods[i][0]))
		new_db = self.options.path_to_db_io+"/"+db_name+"_myopic_"+str(time_periods[i-1][0])+"_"+str(time_periods[i][0])+self.options.output[loc2:]
		...
		cur.execute("DELETE FROM Efficiency WHERE tech IN (SELECT tech FROM LifetimeProcess WHERE \
					 LifetimeProcess.life_process+LifetimeProcess.vintage<="+str(time_periods[i-1][0])+") \
					 AND vintage IN (SELECT vintage FROM LifetimeProcess WHERE LifetimeProcess.life_process+\
					 LifetimeProcess.vintage<="+str(time_periods[i-1][0])+");")
		
		# Delete row from Efficiency if (t,v) retires at the begining of current period (which is time_periods[i][0])
		cur.execute("DELETE FROM Efficiency WHERE tech IN (SELECT tech FROM LifetimeTech WHERE \
					 LifetimeTech.life+Efficiency.vintage<="+str(time_periods[i-1][0])+") AND \
		...
		cur.execute("DELETE FROM Efficiency WHERE tech IN (SELECT tech FROM Efficiency WHERE \
					40+Efficiency.vintage<="+str(time_periods[i-1][0])+") AND \
    	...
    	# ---------------------------------------------------------------
    	# Sufficiecnt changes were made on the Efficiecny table. 
		...
		for table in tables_group1:
			cur.execute("DELETE FROM "+table +" WHERE periods != "+str(time_periods[i-1][0])+" AND periods != "+str(time_periods[i][0])+";")
		...
		if i!=1:
			df_new_ExistingCapacity = pd.read_sql_query("SELECT regions, tech, vintage, capacity FROM Output_V_Capacity \
														 ...
														 vintage == "+str(time_periods[i-2][0])+";", con_org) 
			...
			cur.execute("INSERT INTO Efficiency \
						 SELECT DISTINCT regions,input_comm,tech,"+str(time_periods[i-1][0])+ \
						 ",output_comm,efficiency,eff_notes FROM Efficiency WHERE tech NOT IN (SELECT tech \
						 FROM Efficiency WHERE vintage<"+str(time_periods[0][0])+") AND tech NOT IN (SELECT \
						 tech FROM Efficiency WHERE vintage="+str(time_periods[i-1][0])+");")			
			...
			# Discard the results associated with str(time_periods[i-1][0]) period in rolling horizon fashion. Otherwise, UNIQUE CONSTRAINT error is thrown.
			# Re Output_Costs, a delete is not needed because in pformat_results.py, future periods costs get added to what is already in the table
   			cur_org.execute("DELETE FROM Output_CapacityByPeriodAndTech WHERE scenario="+"'"+str(self.options.scenario)+"' AND t_periods="+str(time_periods[i-1][0]))
   			cur_org.execute("DELETE FROM Output_Emissions WHERE scenario="+"'"+str(self.options.scenario)+"' AND t_periods="+str(time_periods[i-1][0]))
   			cur_org.execute("DELETE FROM Output_VFlow_In WHERE scenario="+"'"+str(self.options.scenario)+"' AND t_periods="+str(time_periods[i-1][0]))
   			cur_org.execute("DELETE FROM Output_VFlow_Out WHERE scenario="+"'"+str(self.options.scenario)+"' AND t_periods="+str(time_periods[i-1][0]))
   			cur_org.execute("DELETE FROM Output_V_Capacity WHERE scenario="+"'"+str(self.options.scenario)+"' AND vintage="+str(time_periods[i-1][0]))
   			cur_org.execute("DELETE FROM Output_Curtailment WHERE scenario="+"'"+str(self.options.scenario)+"' AND t_periods="+str(time_periods[i-1][0]))
   			con_org.commit()
			...
			try:
				...
				if 'vintage' in names:				
					if table[0]!='ExistingCapacity':
						...
						names = list(map(lambda x: x[0], cursor.description))
						names = [str(time_periods[i-1][0]) if x=='vintage' else x for x in names]
						cur.execute("INSERT INTO "+table[0]+" SELECT DISTINCT "+",".join(names)+\
									 " FROM "+table[0]+" WHERE tech NOT IN (SELECT tech FROM "+table[0]+\
									 " WHERE vintage<"+str(time_periods[0][0])+") AND tech NOT IN (SELECT tech FROM "+\
									 table[0]+" WHERE vintage="+str(time_periods[i-1][0])+");"
									 )		
					# For these two table we only want current vintages. 
					if table[0] == 'CostInvest' or table[0] == 'DiscountRate':
						cur.execute("DELETE FROM "+str(table[0])+" WHERE vintage!="+str(time_periods[i-1][0])+" AND vintage!="+str(time_periods[i][0])+";")
		...
		cur.execute("UPDATE `time_periods` SET flag='e' WHERE t_periods < "+str(time_periods[i-1][0]))
		cur.execute("VACUUM")
		con.commit()
		con.close()
    	...
    	# ---------------------------------------------------------------
		new_config = os.getcwd()+"/temoa_model/config_sample_myopic_"+str(time_periods[i-1][0])+"_"+str(time_periods[i][0])
		ifile = open(os.getcwd()+"/temoa_model/config_sample")
		ofile = open(new_config,'w')
		for line in ifile:
			new_line = line.replace("--input=data_files/"+db_name, "--input=data_files/"+db_name+"_myopic_"+str(time_periods[i-1][0])+"_"+str(time_periods[i][0]))
		...
		ifile.close()
		ofile.close()
		os.system("python temoa_model/ --config=temoa_model/config_sample_myopic_"+str(time_periods[i-1][0])+"_"+str(time_periods[i][0]))
		# delete the temporary config file
		os.system("rm -rf temoa_model/config_sample_myopic_"+str(time_periods[i-1][0])+"_"+str(time_periods[i][0]))
