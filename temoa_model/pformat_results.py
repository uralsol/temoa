...
def pformat_results ( pyomo_instance, pyomo_result, options ):
	...
	if hasattr(options, 'file_location') and '_myopic' in options.file_location:
	...
	if hasattr(options, 'file_location') and '_myopic' not in options.file_location:
		...
		for table in svars.keys() :
			if table in tables :
				...
				for val in cur :
					...
					if hasattr(options, 'file_location') and options.scenario == val[0] and '_myopic' not in options.file_location:
