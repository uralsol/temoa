...
def parameters(dm_instance, iaux, time_fut, instance, dV_Capacity, price_data, alpha, sigma, P_theta, eps, PRICE_RANGE):
    ...
    for p, s, d, dem  in sorted(instance.V_Demand.keys()):
        ...
        if p == iaux:
            ...
            print >> price_data, "%10g  %15s    %5s    %5s   %10g    %10g" %\
                        (p, s, d, dem, phi, minESDemand)
...
def solve_dm(model, dat):
    ...
    optimizer = SolverFactory('cplex')  #Defining the optimization solver
    ...
    for item in sorted(ConstantDemandConstraint.items()):
        ...
        (item[0][0], item[0][1], item[0][2], item[0][3], instance.dual[item[1]],1.1)
...
def find_price(dm_instance, instance):
    ...
    print >> price_data, "    ;\n"
    print >> price_data, "param num_demand_segments := %d ; # number of segments in the demand range" % DEMAND_SEGMENTS
    print >> price_data, "param alpha := %f ; # productivity of energy efficiency" % alpha
    print >> price_data, "param sigma := %f ; # elasticity of substitution" % sigma
    print >> price_data, "param eps := %f ; # energy service demand elasticity" % eps
    print >> price_data, "param efficiencyPrice := %f ; # energy service demand elasticity" % P_theta
    print >> price_data, "param beta := %f ; # efficiency credit" % beta
...
def temoa_efficiency(dat, dat2):
    ...
    def TotalWelfare_rule ( M ):
        ...
        producer_costs = sum( PeriodCost_rule(M, p) for p in M.time_optimize )
        EEfficiency_cost = value(M.beta) * value(M.efficiencyPrice) * sum(M.efficiencyDemand[S_p, S_s, S_d, S_c] for S_p, S_s, S_d, S_c in M.DemandConstraint_psdc)
...
def efficiency_run():
    ...
    # Objective
    print >>fp, '\"Objective function is:', value(instance.TotalWelfare), '\"'
    ...
    if e == 'co2')
    print >> dm, "year season time demand elecDemand elecPrice ESDemand ESPrice efficiencyDemand CO2_emissions"
    for p, s, d, dem  in sorted(instance.DemandConstraint_psdc.keys()):
        key = p, s, d, dem
        #IP()
